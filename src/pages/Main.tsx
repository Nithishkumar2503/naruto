...
const mainList = mainItem.map((val) => (
  <div
    key={val.id}
    className="lg:h-52 bg-white lg:w-52 rounded-lg w-40 h-40  hover:scale-105 shadow-lg shadow-primary  bg-whiteo text-center lg:p-2 "
  >
    <a href={`/${val?.title?.toLocaleLowerCase()}`} className="">
      <img
        src={val.img}
        className="mx-auto lg:h-40  lg:w-40 w-26 h-26 rounded-lg"
        alt=""
      />
  ...
  return (
    <div className="h-screen ">
      <img
        ...
        className="mx-auto lg:w-80 w-40  bg-background "
        alt=""
      />
